...
class PrintJob:
    ...
    def reorder(self, rules: dict[int, Rule]) -> int:
        reordered_list: list[int] = []
        for page_number in self.page_numbers:
            current_rule = rules.get(page_number)

            if len(reordered_list) == 0:
                reordered_list.append(page_number)
                continue
            ...
            for position in range(len(reordered_list) + 1):
                is_disjoint_before = set(current_rule.numbers_before).isdisjoint(
                    set(reordered_list[position:])
                )
                is_disjoint_after = set(current_rule.numbers_after).isdisjoint(
                    set(reordered_list[:position])
                )
                if is_disjoint_before and is_disjoint_after:
                    reordered_list.insert(position, page_number)
                    break
        if len(reordered_list) != len(self.page_numbers):
            raise (ValueError)
        return reordered_list[int((len(reordered_list) - 1) / 2)]
...
sum_middle_pages = 0
sum_middle_pages_reordered = 0

for printjob in printjobs:
    if printjob.check_if_correct(rules):
        sum_middle_pages += printjob.middle_page
    else:
        sum_middle_pages_reordered += printjob.reorder(rules)


print(sum_middle_pages_reordered)
print(len(rules))
